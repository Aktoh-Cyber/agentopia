...
class GitHubMCPClient:
    ...
    async def _github_request(self, method: str, endpoint: str, data: Optional[Dict] = None) -> Dict:
    ...
    async def get_branch_sha(self, branch: str) -> str:
        ...
        ref_data = await self._github_request("GET", f"/repos/{self.repo_owner}/{self.repo_name}/git/refs/heads/{branch}")
    ...
    async def create_branch(self, branch_name: str, base_branch: str = None) -> str:
        ...
        try:
            await self._github_request("POST", f"/repos/{self.repo_owner}/{self.repo_name}/git/refs", branch_data)
            return branch_name
    ...
    async def get_file_content(self, file_path: str, branch: str = None) -> Optional[Dict]:
        ...
        try:
            file_data = await self._github_request("GET", f"/repos/{self.repo_owner}/{self.repo_name}/contents/{file_path}?ref={branch}")
            return {
    ...
    async def commit_files(self, files: List[Dict[str, str]], branch: str, commit_message: str) -> str:
        ...
        commit_data = await self._github_request("GET", f"/repos/{self.repo_owner}/{self.repo_name}/git/commits/{branch_sha}")
        ...
        for file_info in files:
            ...
            blob_response = await self._github_request("POST", f"/repos/{self.repo_owner}/{self.repo_name}/git/blobs", blob_data)
        ...
        tree_response = await self._github_request("POST", f"/repos/{self.repo_owner}/{self.repo_name}/git/trees", tree_data)
        ...
        commit_response = await self._github_request("POST", f"/repos/{self.repo_owner}/{self.repo_name}/git/commits", commit_data)
        ...
        await self._github_request("PATCH", f"/repos/{self.repo_owner}/{self.repo_name}/git/refs/heads/{branch}", update_data)
    ...
    async def create_pull_request(self, 
        ...
        pr_response = await self._github_request("POST", f"/repos/{self.repo_owner}/{self.repo_name}/pulls", pr_data)
