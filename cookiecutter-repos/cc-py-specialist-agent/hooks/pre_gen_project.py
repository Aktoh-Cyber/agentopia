#!/usr/bin/env python
"""Pre-generation hook for Python specialist template."""
...
def validate_priority(priority):
    ...
    try:
        p = int(priority)
        if p < 1 or p > 10:
            print(f"ERROR: Priority must be between 1 and 10, got {p}")
            sys.exit(1)
        return True
    except ValueError:
        print(f"ERROR: Priority must be a number, got {priority}")
        sys.exit(1)
...
def main():
    ...
    print("✅ All validations passed for Python specialist!")
