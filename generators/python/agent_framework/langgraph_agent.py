    ...
    def setup_supervisor_pattern(self):
        ...
        async def supervisor_node(state: AgentState) -> AgentState:
            prompt = ChatPromptTemplate.from_messages([
                ("system", """You are a supervisor agent managing multiple specialized agents. 
...
Respond with JSON: {{"next_agent": "agent_name", "instructions": "specific instructions", "reasoning": "why this agent"}}"""),
            ...
            try:
                ...
                decision = {
                    "next_agent": self.config["agents"][0]["name"] if self.config["agents"] else "end",
                    "instructions": "Proceed with default processing",
                    "reasoning": "Default routing"
                }
            
            state.current_agent = decision["next_agent"]
            state.add_message(AIMessage(content=f"Supervisor: Routing to {decision['next_agent']}. {decision['reasoning']}"))
        ...
        for agent_config in self.config["agents"]:
            ...
            async def create_agent_node(config):
                async def agent_node(state: AgentState) -> AgentState:
                    prompt = ChatPromptTemplate.from_messages([
                        ("system", """You are {agent_name}, a specialized agent with expertise in: {expertise}
        ...
        for agent_config in self.config["agents"]:
            ...
            async def create_network_node(config):
                async def network_node(state: AgentState) -> AgentState:
                    prompt = ChatPromptTemplate.from_messages([
                        ("system", """You are {agent_name} in a network of peer agents. You can communicate with any other agent.
...
Respond with JSON: {{"action": "process|consult|complete", "target_agent": "agent_name_if_consulting", "response": "your response"}}"""),
                    ...
                    available_peers = ", ".join([
                        a["name"] for a in self.config["agents"] 
                    ...
                    state.add_message(AIMessage(content=f"{config['name']}: {decision['response']}"))
    ...
    def setup_router_pattern(self):
        ...
        async def router_node(state: AgentState) -> AgentState:
            prompt = ChatPromptTemplate.from_messages([
                ("system", """You are a router agent. Analyze the task and route it to the most appropriate specialist.
...
Respond with JSON: {{"selected_agent": "agent_name", "confidence": 0-100, "reasoning": "why this agent"}}"""),
            ...
            try:
                ...
                decision = {
                    "selected_agent": self.config["agents"][0]["name"] if self.config["agents"] else "end",
                    "confidence": 50,
                    "reasoning": "Default routing"
                }
        ...
        for agent_config in self.config["agents"]:
            ...
            async def create_specialist_node(config):
                async def specialist_node(state: AgentState) -> AgentState:
                    prompt = ChatPromptTemplate.from_messages([
                        ("system", """You are {agent_name}, selected for your expertise in: {expertise}
    ...
    def setup_committee_pattern(self):
        ...
        async def coordinator_node(state: AgentState) -> AgentState:
            if state.iteration == 0:
                ...
                state.current_agent = self.config["agents"][0]["name"] if self.config["agents"] else "aggregator"
                state.add_message(AIMessage(content="Coordinator: Delegating to committee members for diverse perspectives"))
            else:
        ...
        for i, agent_config in enumerate(self.config["agents"]):
            ...
            async def create_committee_node(config, index):
                async def committee_node(state: AgentState) -> AgentState:
                    prompt = ChatPromptTemplate.from_messages([
                        ("system", """You are {agent_name}, a committee member with expertise in: {expertise}
...
Provide your independent analysis and recommendation:"""),
        ...
        async def aggregator_node(state: AgentState) -> AgentState:
            ...
            prompt = ChatPromptTemplate.from_messages([
                ("system", """You are an aggregator combining perspectives from committee members.
...
Synthesize these perspectives into a comprehensive solution:"""),
    ...
    def setup_reflection_pattern(self):
        ...
        async def worker_node(state: AgentState) -> AgentState:
            prompt = ChatPromptTemplate.from_messages([
                ("system", """You are a worker agent. Process the following task:
...
Provide your solution:"""),
        ...
        async def reflector_node(state: AgentState) -> AgentState:
            ...
            prompt = ChatPromptTemplate.from_messages([
                ("system", """You are a reflector agent. Analyze the worker's attempt and decide if it's acceptable or needs improvement.
...
Respond with JSON: {{"satisfactory": true/false, "feedback": "your feedback", "continue": true/false}}"""),
            ...
            if decision["satisfactory"] or not decision["continue"] or state.iteration >= self.config["maxIterations"]:
                state.current_agent = "end"
            else:
        ...
        for i, agent_config in enumerate(self.config["agents"]):
            ...
            async def create_pipeline_node(config, stage, total_stages, next_node):
                async def pipeline_node(state: AgentState) -> AgentState:
                    prompt = ChatPromptTemplate.from_messages([
                        ("system", """You are {agent_name} in a processing pipeline. 
Your role: {expertise}
...
Process the input and prepare output for the next stage:"""),
                    ...
                    previous_output = state.task if stage == 1 else (
                        state.messages[-1].content if state.messages else state.task
                    )
            ...
            self.graph.add_node(
                agent_name, 
                await create_pipeline_node(agent_config, i + 1, len(self.config["agents"]), next_agent)
            )
    ...
    def setup_autonomous_pattern(self):
        ...
        async def autonomous_node(state: AgentState) -> AgentState:
            prompt = ChatPromptTemplate.from_messages([
                ("system", """You are an autonomous agent. You can decide your own next actions based on the current state.
...
Respond with JSON: {{"action": "chosen_action", "reasoning": "why", "output": "any output from this step"}}"""),
    ...
    def add_pipeline_edges(self):
        ...
        for i in range(len(self.config["agents"]) - 1):
            self.graph.add_edge(
                self.config["agents"][i]["name"],
                self.config["agents"][i + 1]["name"]
            )
